...
import { jobs, jobSnapshots, syncRuns, jobSnapshotDiffs, emailJobs, emailEvents, emailSuppression, jobFlags } from "./schema.js";
...
export interface JobFlag {
  ...
  jobId?: string;
  company?: string;
  flagType: "problematic_job" | "problematic_company" | "spam" | "duplicate";
  reason?: string;
  metadata?: Record<string, unknown>;
  createdAt: Date;
}

export interface JobFlagStats {
  jobId?: string;
  company?: string;
  flagType: string;
  count: number;
...
/**
 * Add a flag (vote) for a job or company as problematic/spam/duplicate.
 * If the user already has a flag of this type for this job/company, it's skipped (idempotent).
 */
export async function addJobFlag(
  userId: string,
  flagType: "problematic_job" | "problematic_company" | "spam" | "duplicate",
  options: {
    jobId?: string;
  ...
  const id = typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `flag-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  
  // Check if user already flagged this job/company with this type
  const existing = await db
    .select()
    .from(jobFlags)
    .where(
      and(
        ...
        eq(jobFlags.flagType, flagType),
        options.jobId ? eq(jobFlags.jobId, options.jobId) : sql`TRUE`,
        options.company ? eq(jobFlags.company, options.company) : sql`TRUE`
      )
    )
    .limit(1);

  if (existing.length > 0) {
    // Already flagged, return existing record (idempotent)
    return existing[0] as JobFlag;
  }
  ...
  await db.insert(jobFlags).values(record as any);
  return record as JobFlag;
}

/**
 * Remove a flag (vote) for a job/company by a specific user.
 * Returns true if a flag was removed, false if no matching flag found.
 */
export async function removeJobFlag(
  userId: string,
  flagType: "problematic_job" | "problematic_company" | "spam" | "duplicate",
  options: {
    jobId?: string;
  ...
  const result = await db
    ...
    .where(
      and(
        eq(jobFlags.userId, userId),
        eq(jobFlags.flagType, flagType),
        options.jobId ? eq(jobFlags.jobId, options.jobId) : sql`${jobFlags.jobId} IS NULL`,
        options.company ? eq(jobFlags.company, options.company) : sql`${jobFlags.company} IS NULL`
      )
    );
...
/**
 * Get flag statistics for a job or company.
 * Returns counts grouped by flag type.
 */
export async function getJobFlagStats(options: { jobId?: string; company?: string }): Promise<JobFlagStats[]> {
  const rows = await db
    .select({
      jobId: jobFlags.jobId,
      company: jobFlags.company,
      flagType: jobFlags.flagType,
    ...
    .groupBy(jobFlags.jobId, jobFlags.company, jobFlags.flagType);

  return rows as JobFlagStats[];
}

/**
 * Check if a job is flagged as problematic (by threshold of users).
 */
export async function isJobFlaggedAsProblematic(
  jobId: string,
  threshold = 2
): Promise<boolean> {
  const rows = await db
    ...
    .where(
      and(
        ...
        sql`${jobFlags.flagType} IN ('problematic_job', 'problematic_company')`
      )
    )
    .groupBy(jobFlags.jobId);

  if (rows.length === 0) return false;
  return (rows[0] as any).count >= threshold;
}

/**
 * Check if a company is flagged as problematic (by threshold of users).
 */
export async function isCompanyFlaggedAsProblematic(
  company: string,
  threshold = 2
): Promise<boolean> {
  const rows = await db
    ...
    .where(
      and(
        eq(jobFlags.company, company),
        eq(jobFlags.flagType, "problematic_company")
      )
    )
    .groupBy(jobFlags.company);
