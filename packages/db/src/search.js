...
export async function searchJobs(query, limit = 50, offset = 0) {
    const whereClauses = [eq(jobs.status, "open")];
    if (query.countries && query.countries.length) {
        const countryConds = query.countries.map((c) => sql `LOWER(${jobs.locations}::text) LIKE ${"%" + String(c).toLowerCase() + "%"}`);
        whereClauses.push(or(...countryConds));
    }
    if (query.cities && query.cities.length) {
        const cityConds = query.cities.map((c) => sql `LOWER(${jobs.locations}::text) LIKE ${"%" + String(c).toLowerCase() + "%"}`);
        whereClauses.push(or(...cityConds));
    }
    if (query.levels && query.levels.length) {
        const levelConds = query.levels.map((l) => sql `${jobs.level} = ${l}`);
        whereClauses.push(or(...levelConds));
    }
    if (query.workModes && query.workModes.length) {
        const modeConds = query.workModes.map((m) => sql `LOWER(${jobs.workModes}::text) LIKE ${"%" + String(m).toLowerCase() + "%"}`);
        whereClauses.push(or(...modeConds));
    }
    if (query.preferredTechnologies && query.preferredTechnologies.length) {
        const techConds = query.preferredTechnologies.map((t) => sql `LOWER(${jobs.technologies}::text) LIKE ${"%" + String(t).toLowerCase() + "%"}`);
    ...
    return rows.map((r) => ({
        ...
        descriptionText: r.descriptionText ?? undefined,
        descriptionHtml: r.descriptionHtml ?? undefined,
