...
const board = process.argv[2] ?? process.env.ASHBY_JOB_BOARD;
if (!board) {
  throw new Error("Usage: pnpm --filter @job-hunter/worker sync:ashby <job-board> or set ASHBY_JOB_BOARD");
}
...
try {
  ...
  if (isSuspicious) {
    ...
    try {
      ...
      if (resendKey && alertTo) {
        try {
          await sendSuspicionEmail(resendKey, alertTo.split(","), runId, board, predictedChanged, predictedRatio, csvPath);
  ...
  for (const job of jobs) {
    ...
    if (processed % progressUpdateInterval === 0) {
      try {
        await updateSyncRunCounts(runId, processed, changed, undefined, isSuspicious, anomalyScore);
      } catch (e) {
  ...
  await updateSyncRunCounts(runId, jobs.length, changed, closed, isSuspicious, anomalyScore);
...
async function sendSuspicionEmail(apiKey: string, to: string[], runId: string, board: string, changedCount: number, ratio: number, csvPath?: string) {
  ...
  _finished = true;
  console.error(`Ashby sync failed: ${msg}`);
  throw err;
} finally {
  if (runId && !_finished) {
    try {
